...
public class Karakter {
    ...
    public boolean memilikiItem(String namaItem) {
        if (namaItem == null) {
            return false;
        }
        for (Item item : this.inventory.getItems()) {
            if (item != null && item.getNamaItem().equalsIgnoreCase(namaItem)) {
                return true;
            }
        }
        if (this.itemAktif != null && this.itemAktif.getNamaItem().equalsIgnoreCase(namaItem)) {
            return true;
        }
        return this.armorAktif != null && this.armorAktif.getNamaItem().equalsIgnoreCase(namaItem);
    }
}
